...
var ErrorFollowingPreviousScan = errors.New("scanning not allowed immediately following previous scan")
...
const (
	...
	// Signal
	DeviceWirelessPropertiesChanged = DeviceWirelessInterface + ".PropertiesChanged"
)
...
type DeviceWireless interface {
	...
	//
	// Deprecated: use GetAllAccessPoints() instead
...
func (d *deviceWireless) RequestScan() error {
	...
	err := d.obj.Call(DeviceWirelessRequestScan, 0, options).Store()
	if err != nil {
		if strings.Contains(err.Error(), "not allowed immediately following previous scan") {
			return ErrorFollowingPreviousScan
		}
	}
