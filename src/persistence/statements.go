...
const QuerySelectLogHistoryByAppKeyStageAndType = `SELECT log FROM LogHistory WHERE app_key = ? AND stage = ? AND log_type = ?`
...
const QueryUpsertLogHistoryEntry = `INSERT INTO LogHistory(app_name, app_key, stage, log_type, log) VALUES (?, ?, ?, ?, ?) ON conflict(app_name, app_key, stage, log_type) do update set
log = excluded.log
`
