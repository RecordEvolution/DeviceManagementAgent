...
func main() {
	...
	fmt.Printf("testing reagent...\n")

	osReleaseLatest, err := system.GetOSReleaseLatest()
	if err != nil {
		fmt.Printf("failed to GetOSReleaseLatest")
	}
	fmt.Printf(osReleaseLatest["buildtime"]+"\n")
	fmt.Printf(osReleaseLatest["download"]+"\n")
	fmt.Printf(osReleaseLatest["file"]+"\n")
	fmt.Printf(osReleaseLatest["sha256"]+"\n")
	fmt.Printf(osReleaseLatest["version"]+"\n")

	osReleaseCurrent, err := system.GetOSReleaseCurrent()
	if err != nil {
		fmt.Printf("failed to GetOSReleaseCurrent")
	}
	fmt.Printf(osReleaseCurrent["NAME"]+"\n")
	fmt.Printf(osReleaseCurrent["VERSION"]+"\n")
	fmt.Printf(osReleaseCurrent["VERSION_ID"]+"\n")
	fmt.Printf(osReleaseCurrent["PRETTY_NAME"]+"\n")
	...
	if err != nil {
		fmt.Printf("failed to download bundle\n")
	}

	progressCall := func(percent uint64) {

	}
	fmt.Printf("starting to install bundle\n")
	err = system.InstallOSUpdate(progressCall)
	if err != nil {
		fmt.Printf("failed to install bundle\n")
	}
